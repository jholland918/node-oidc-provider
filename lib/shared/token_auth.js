  ...
  return {
    ...
    middleware: [
      ...
      async function findClientId(ctx, next) {
        ...
        console.log(`[jmh] token_auth.js findClientId()`);
        console.log(`[jmh]   clientId[${clientId}]`);
        console.log(`[jmh]   clientAssertion[${clientAssertion}]`);
        console.log(`[jmh]   clientAssertionType[${clientAssertionType}]`);
        console.log(`[jmh]   clientSecret[${clientSecret}]`);

        // If basic auth exists, enforce only ['client_secret_basic', 'client_secret_post'] auth methods...
        if (ctx.headers.authorization !== undefined) {
          console.log(`[jmh] ctx.headers.authorization found [${ctx.headers.authorization}]...`);
          ...
          console.log(`[jmh] Basic auth found, therefore ctx.oidc.authorization.methods [${JSON.stringify(ctx.oidc.authorization.methods)}] are required`);
        } else if (clientId !== undefined) {
          console.log(`[jmh] ctx.headers.authorization NOT found, using clientId[${clientId}] instead...`);
          ...
          ctx.oidc.authorization.methods = clientSecret
            ...
            console.log(`[jmh] The clientSecret is [${clientSecret}], therefore the ctx.oidc.authorization.methods[${ctx.oidc.authorization.methods}] are required`);
        }

        if (clientAssertion !== undefined) {
          console.log(`[jmh]   if (clientAssertion !== undefined) > clientAssertion[${clientAssertion}]`);
          ...
          try {
            ...
            allDecoded = JWT.decode(clientAssertion);
          } catch (err) {
            throw new InvalidRequest('invalid client_assertion format');
          }

          if (clientId && sub !== clientId) {
            //throw new InvalidRequest(`clientAssertion[${clientAssertion}]: subject[${sub}] of client_assertion must be the same as client_id[${clientId}] provided in the body`); //jmh
            throw new InvalidRequest(allDecoded);
          }
      ...
      async function loadClient(ctx, next) {
        console.log(`[jmh] token_auth.js loadClient()`);
        ...
        console.log(`[jmh] found client [${ctx.oidc.authorization.clientId}]...`);
