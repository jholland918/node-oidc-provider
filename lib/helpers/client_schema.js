        ...
        if (this[prop] !== undefined) {
          ...
          if (isAry && !this[prop].every((val) => only[method](val))) {
            ...
            if (length) {
              ...
              this.invalidate(`JMH[4] error1[${error1}] ENUM[${JSON.stringify(ENUM)}] ARYS[${JSON.stringify(ARYS)}] method[${JSON.stringify(method)}] - ${prop} must not be provided (no values are allowed)`);
            }
          }
        }
      });
