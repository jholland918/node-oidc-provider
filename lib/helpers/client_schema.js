  ...
  class Schema {
    constructor(
      ...
      ['revocation', 'introspection'].forEach((endpoint) => {
        ...
        //jmh//console.log(`[jmh] checking metadata for endpoint[${endpoint}]...`);

        if (metadata[`${endpoint}_endpoint_auth_method`] === undefined) {
          //jmh//console.log(`[jmh] metadata[${endpoint}_endpoint_auth_method] not found, falling back to token_endpoint_auth_method[${JSON.stringify(metadata.token_endpoint_auth_method || configuration.clientDefaults.token_endpoint_auth_method)}]`);
        ...
        if (metadata[`${endpoint}_endpoint_auth_signing_alg`] === undefined && metadata.token_endpoint_auth_signing_alg) {
          //jmh//console.log(`[jmh] metadata[${endpoint}_endpoint_auth_signing_alg] not found, falling back to token_endpoint_auth_signing_alg[${JSON.stringify(metadata.token_endpoint_auth_signing_alg)}]`);
