...
module.exports = function getSchema(provider) {
  console.info(`[jmh] getSchema(provider [${JSON.stringify(provider)}])`);
  ...
  const ENUM = {
    ...
    id_token_signed_response_alg: (metadata) => {
      console.log(`[jmh] ENUM.id_token_signed_response_alg: (metadata) => {...}`);

      console.log(`[jmh] checking metadata.response_types for 'id_token': metadata.response_types[${JSON.stringify(metadata.response_types)}] `);
      if (!metadata.response_types.join(' ').includes('id_token')) {
        console.log(`[jmh] id_token NOT found, returning configuration.idTokenSigningAlgValues[${JSON.stringify(configuration.idTokenSigningAlgValues)}]...`);
        return configuration.idTokenSigningAlgValues;
      }

      console.log(`[jmh] id_token WAS found, returning configuration.idTokenSigningAlgValues[${JSON.stringify(configuration.idTokenSigningAlgValues)}] without 'none'...`);
  ...
  class Schema {
    constructor(
      ...
      console.log(`[jmh] Creating new Schema(metadata[${JSON.stringify(metadata)}], ctx[${JSON.stringify(ctx)}], processCustomMetadata[${JSON.stringify(processCustomMetadata)}])`);
      
      // unless explicitly provided use token_* values
      ['revocation', 'introspection'].forEach((endpoint) => {

        console.log(`[jmh] checking metadata for endpoint[${endpoint}]...`);

        if (metadata[`${endpoint}_endpoint_auth_method`] === undefined) {
          console.log(`[jmh] metadata[${endpoint}_endpoint_auth_method] not found, falling back to token_endpoint_auth_method[${JSON.stringify(metadata.token_endpoint_auth_method || configuration.clientDefaults.token_endpoint_auth_method)}]`);
        ...
        if (metadata[`${endpoint}_endpoint_auth_signing_alg`] === undefined && metadata.token_endpoint_auth_signing_alg) {
          console.log(`[jmh] metadata[${endpoint}_endpoint_auth_signing_alg] not found, falling back to token_endpoint_auth_signing_alg[${JSON.stringify(metadata.token_endpoint_auth_signing_alg)}]`);
      ...
      console.log(`[jmh] Running Object.assign...`);
      console.log(`[jmh]   DEFAULT[${JSON.stringify(DEFAULT)}]`);
      console.log(`[jmh]   RECOGNIZED_METADATA[${JSON.stringify(RECOGNIZED_METADATA)}]`);
      console.log(`[jmh]   configuration.extraClientMetadata.properties[${JSON.stringify(configuration.extraClientMetadata.properties)}]`);
      ...
      console.log(`[jmh] assigned to this[${JSON.stringify(debug1)}]`);
      ...
      console.log('[jmh] going into the dreaded enums!!!1');
      this.enums();
      this.webUris();
      this.scopes();
    ...
    required() {
      ...
      if (provider.Client.needsSecret(this, tracker1)) {
        tracker += 'b;';
      ...
      checked.forEach((prop) => {
        if (!this[prop]) {
          tracker += `c prop[${prop}];`;
          this.invalidate(`${prop} is mandatory property - provider.Client[${JSON.stringify(provider.Client)}] tracker1[${JSON.stringify(tracker1)}]`);
        }
      });
    ...
    enums() {
      console.log(`[jmh] enums()`);
      console.log(`[jmh]   ENUM[${JSON.stringify(ENUM)}]`);

      Object.entries(ENUM).forEach(([prop, fn]) => {
        console.log(`[jmh]   Current prop[${prop}]`);

        const only = fn(this);

        console.log(`[jmh] ENUM only[${JSON.stringify(only)}]`);

        if (this[prop] !== undefined) {
          console.log(`[jmh]   this.[${prop}] is defined...`);
          ...
          if (only instanceof Set) {
            ({ size: length } = only);
            method = 'has';
            console.log(`[jmh] 'only' is a Set, using 'has' method...`);
          } else {
            ({ length } = only);
            method = 'includes';
            console.log(`[jmh] 'only' is NOT a Set, using 'includes' method...`);
          }

          if (isAry && !this[prop].every((val) => only[method](val))) {
            if (length) {
              this.invalidate(`JMH[1] ${prop} can only contain ${formatters.formatList([...only], { type: 'disjunction' })}`);
            } else {
              this.invalidate(`JMH[2] ${prop} must be empty (no values are allowed)`);
            }
          } else if (!isAry && !only[method](this[prop])) {
            if (length) {
              this.invalidate(`JMH[3] ${prop} must be ${formatters.formatList([...only], { type: 'disjunction' })}`);
            } else {
              let error1 = '';
              try {
              ...
              this.invalidate(`JMH[4] - ${prop} must not be provided (no values are allowed)`);
            }
          }
        }
      });
