...
module.exports = function getClient(provider) {
  ...
  class Client {
    constructor(metadata, ctx) {
      console.log(`[jmh] Creating Client object...`);
    ...
    static async find(id) {
      console.log(`[jmh] lib/models/client.js find(id [${id}])`);
      if (typeof id !== 'string') {
        return undefined;
      }
      ...
      if (staticCache.has(id)) {
        ...
        console.log('[jmh] found client from staticCache...');
      ...
      if (!client) {
        console.log('[jmh] did not find client in dynamicCache, adding it to dynamicCache...');
        client = await add(properties, { store: false });
        dynamicCache.set(propHash, client);
      }

      console.log('[jmh] //lib/models/client.js find()');
      return client;
    }

    static needsSecret(metadata, tracker1) {
      if (!tracker1) {
        tracker1 = [];
      }
