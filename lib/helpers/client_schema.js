...
module.exports = function getSchema(provider) {
  ...
  const ENUM = {
    ...
    id_token_signed_response_alg: (metadata) => {
      //jmh//console.log(`[jmh] ENUM.id_token_signed_response_alg: (metadata) => {...}`);

      //jmh//console.log(`[jmh] checking metadata.response_types for 'id_token': metadata.response_types[${JSON.stringify(metadata.response_types)}] `);
      if (!metadata.response_types.join(' ').includes('id_token')) {
        //jmh//console.log(`[jmh] id_token NOT found, returning configuration.idTokenSigningAlgValues[${JSON.stringify(configuration.idTokenSigningAlgValues)}]...`);
        return configuration.idTokenSigningAlgValues;
      }

      //jmh//console.log(`[jmh] id_token WAS found, returning configuration.idTokenSigningAlgValues[${JSON.stringify(configuration.idTokenSigningAlgValues)}] without 'none'...`);
  ...
  class Schema {
    ...
    enums() {
      //jmh//console.log(`[jmh] enums()`);
      //jmh//console.log(`[jmh]   ENUM[${JSON.stringify(ENUM)}]`);

      Object.entries(ENUM).forEach(([prop, fn]) => {
        //jmh//console.log(`[jmh]   Current prop[${prop}]`);

        const only = fn(this);

        //jmh//console.log(`[jmh] ENUM only[${JSON.stringify(only)}]`);

        if (this[prop] !== undefined) {
          //jmh//console.log(`[jmh]   this.[${prop}] is defined...`);
