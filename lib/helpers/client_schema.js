    ...
    constructor(
      ...
      ['revocation', 'introspection'].forEach((endpoint) => {
        ...
        if (metadata[`${endpoint}_endpoint_auth_method`] === undefined) {
          console.log(`[jmh] key[${endpoint}_endpoint_auth_method] not found in metadata...`);

          console.log(`[jmh] key[${endpoint}_endpoint_auth_method] not found in metadata, so we're setting it to the same value as metadata.token_endpoint_auth_method || configuration.clientDefaults.token_endpoint_auth_method`);
        ...
        if (metadata[`${endpoint}_endpoint_auth_signing_alg`] === undefined && metadata.token_endpoint_auth_signing_alg) {
          console.log(`[jmh] key[${endpoint}_endpoint_auth_signing_alg] not found in metadata, so we're setting it to the same value as metadata.token_endpoint_auth_signing_alg`);
